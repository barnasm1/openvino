...
TEST_P(OVHoldersTestOnImportedNetwork, POC_WeightlessProperty) {
    ov::Core core = ov::test::utils::create_core();
    std::stringstream stream;
    std::stringstream stream_weightless;
    const auto& model = function;
    {
        auto compiled_model = core.compile_model(model, target_device);

        ov::AnyMap properties = {ov::hint::compilation_type(ov::hint::CompilationType::NORMAL_WITH_HEADER)};
        auto compiled_model_weightless = core.compile_model(model, target_device, properties);

        compiled_model.export_model(stream);
        compiled_model_weightless.export_model(stream_weightless);

        EXPECT_NE(stream.str(), stream_weightless.str());
    }
